...
def extractInformation(URL):
    ...
    for pair in information:
        if(len(pair) > 1):
            if(pair[0].strip()=='Aired'):
                dates = transformDate(pair[1].strip().replace(
                "\n", "").replace("  ", ""))
                data['AiredFrom'] = dates['AiredFrom']
                data['AiredTo'] = dates['AiredTo']
            else:
...
def extractURLs(URL):
    ...
    soup = BeautifulSoup(r.content, 'html5lib')
    urls = soup.find_all(
        "a", {"class": "hoverinfo_trigger fl-l ml12 mr8"}, href=True)
    for url in urls:
...
def transformDate(date):
    ...
    for i in range(len(listOfDate)):
        ...
        try:
            a = datetime.strptime(
                listOfDate[i], '%b %d, %Y').strftime('%d/%m/%Y')
        except ValueError:
            try:
                a = datetime.strptime(
                    listOfDate[i], '%b, %Y').strftime('%m/%Y')
            except ValueError:
                try:
...
extractURLs("https://myanimelist.net/topanime.php?limit=0")